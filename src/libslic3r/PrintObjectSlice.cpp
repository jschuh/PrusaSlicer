...
void PrintObject::slice_volumes()
{
    ...
    BOOST_LOG_TRIVIAL(debug) << "Slicing volumes - make_slices in parallel - begin";
    {
	    ...
	    tbb::concurrent_vector<ExPolygons> elephant_compensated_layers;
	    tbb::parallel_for(
	        tbb::blocked_range<size_t>(0, m_layers.size()),
			[this, xy_compensation_scaled, elephant_foot_compensation_scaled, &elephant_compensated_layers](const tbb::blocked_range<size_t>& range) {
	            for (size_t layer_id = range.begin(); layer_id < range.end(); ++ layer_id) {
	                m_print->throw_if_canceled();
                    ...
                    if (layer->m_regions.size() == 1) {
	                    ...
	                    if (elfoot > 0) {
                            ...
                            elephant_compensated_layers[layer_id] = to_expolygons(std::move(layerm->slices.surfaces));
							...
							layerm->slices.set(
								union_ex(
									Slic3r::elephant_foot_compensation(
										(delta == 0.f) ? elephant_compensated_layers[layer_id] : offset_ex(elephant_compensated_layers[layer_id], delta),
	                            		layerm->flow(frExternalPerimeter), unscale<double>(elfoot))),
								stInternal);
							if (xy_compensation_scaled < 0.f)
                                elephant_compensated_layers[layer_id] = offset_ex(std::move(elephant_compensated_layers[layer_id]), xy_compensation_scaled);
	                    ...
	                    if (xy_compensation_scaled < 0.f || elfoot > 0.f) {
	                        ...
	                        if (elfoot > 0.f) {
                                elephant_compensated_layers[layer_id] = offset_ex(layer->merged(eps), std::min(xy_compensation_scaled, 0.f) - eps);
								trimming = to_polygons(Slic3r::elephant_foot_compensation(elephant_compensated_layers[layer_id],
	    ...
	    // The Elephant foot has been compensated, therefore these layer's lslices are shrank with the Elephant foot compensation value.
	    // Store the uncompensated values there.
        assert(elephant_compensated_layers.size() <= m_layers.size());
        for (size_t layer_id = 0; layer_id < elephant_compensated_layers.size(); ++layer_id) {
            m_layers[layer_id]->lslices = std::move(elephant_compensated_layers[layer_id]);
		}
	}
